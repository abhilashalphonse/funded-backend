...
// Deliberately small setup endpoint for local MVP testing. Put this behind
// administrator authentication before exposing it outside a test environment.
  ...
  try {
    ...
    if (!accountId || !accountSize || !rules?.phases?.length) {
      return res.status(400).json({
        success: false,
        message: "accountId, accountSize, and rules.phases are required"
      });
    }
    ...
    const simulated = await simulatorEngine.provisionAccount({
...
app.post("/api/accounts/:accountId/retry-command", async (req, res, next) => {
  try {
    ...
    if (!account.commandPending) {
      return res.status(409).json({ success: false, message: "No command is pending for this account" });
    }
